...
class register(basedata):
    ...
    def validate(self):
        # checking for overlapped bitfields and duplicated names
        names = []
        cap = int(self.vsize, 0)
        ovl = cap
        for x in self.fields.sorted(key=lambda x: x._bito, reverse=True):
            # check for bitfield duplicated name
            if x.name in names:
                raise SVD_error('Duplicated bitfield name %s' % (x.name), self)
            else:
                names.append(x.name)
            # check for bitfield bounds
            ofs = int(x.bito, 0)
            wdt = int(x.bitw, 0)
            if ofs < 0 or wdt <= 0 or (ofs + wdt) > cap:
                raise SVD_error('Bitfield %s is out of bounds' % (x.name), self)
            # check for bitfield overlap
            if ovl < (ofs + wdt):
                raise SVD_error('Bitfield %s overlapped' % (x.name), x)
            ovl = ofs
